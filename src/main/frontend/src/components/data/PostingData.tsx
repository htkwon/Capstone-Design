import { Posting } from "../../model/posting";
import { BoardType } from "../../model/board";
import { user1,user2,user3,user4 } from "./UserData";
import { freePosting_1_re1, freePosting_1_re2, freePosting_1_re3, freePosting_2_re1, freePosting_3_re1, freePosting_3_re2 } from "./ReplyData";

const freePosting_1:Posting ={
    type: BoardType.free,
    uuid: "ab34ds89",
    title: "거기 너",
    article: "매점가서 잘 볶아지신 야끼소바빵 사와라",
    imgUrl: ["https://sudachirecipes.com/wp-content/uploads/2021/10/yakisoba-pan-3-1024x683.jpg"],
    writer: user1,
    createdDate: "2023-02-19",
    bookmarks: 0,
    replies: [freePosting_1_re1, freePosting_1_re2, freePosting_1_re3],
    reports: 0
}

const freePosting_2:Posting ={
    type: BoardType.free,
    uuid: "xa20bs89",
    title: "집에 보내주세요",
    article: "집집집집집집ㅈ빚비지비집 아니야...그냥...수업만 끝내줘...",
    writer: user2,
    createdDate: "2023-02-14 ",
    modifiedDate: "2023-02-15",
    bookmarks: 1,
    replies: [freePosting_2_re1],
    reports: 0
}

const freePosting_3:Posting ={
    type: BoardType.free,
    uuid: "gd83ds77",
    title: "와랄랄랄랄랄랄랄랄라",
    article: "라라라라라라라ㅏㄹ라ㅏ라라라라라라랄라ㅏ라라랄라ㅏㄹ",
    writer: user1,
    createdDate: "2023-01-19",
    bookmarks: 0,
    replies: [freePosting_3_re1, freePosting_3_re2],
    reports: 0
}

export {freePosting_1,freePosting_2,freePosting_3,};